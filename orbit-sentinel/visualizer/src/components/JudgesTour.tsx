...
const TOUR_STEPS: TourStep[] = [
  { icon: "🎯", title: "The Problem", subtitle: "This is what we solve", description: "Developers spend 45 min per MR checking blast radius, deployment paths, and historical failures — manually. Traditional CI/CD only checks if code builds, never if it should deploy. This costs teams hours every sprint.", focus: "Three columns: Problem → Solution → Quantified Impact. Reads left to right." },
  { icon: "📈", title: "Impact by the Numbers", subtitle: "Quantified results", description: "Orbit Sentinel saves 45 min per MR. 89% fewer false alarms than CI-only alerts. 88% mitigation success when recommendations are followed. Cross-references all 4 Orbit query types.", focus: "Five quantified metrics — each proven by Orbit graph data." },
  ...
  { icon: "💥", title: "Blast Radius Explorer", subtitle: "NEIGHBORS query", description: "Powered by Orbit's NEIGHBORS query. Shows everything connected to the changed files — downstream services, upstream dependencies, related MRs, and incidents. Interactive D3 force-directed graph.", focus: "Click any node to inspect. Use depth slider to control how far the blast radius extends. Red nodes = high risk." },
  ...
  { icon: "🏁", title: "You've Seen Every View", subtitle: "All 6 views, all 4 queries", description: "You just walked through: Problem → Impact → Verdict → Blast Radius (NEIGHBORS) → Risk (AGGREGATION) → Path Trace → Digital Twin → Simulation → History (TRAVERSAL) → Export. Every Orbit query type powers a live view. Not one line of CI changed.", focus: "Press Space for auto-demo. Arrow keys navigate. ⬇ exports a full HTML report. 👑 restarts this tour. Share the link with ?judge=true." },
];
