...
const TOUR_STEPS: TourStep[] = [
  ...
  { icon: "📄", title: "Full HTML Report", subtitle: "Professional export", description: "Click ⬇ (Export) in the sidebar to generate a complete HTML report of the current analysis — verdict, blast radius, risk dimensions, simulation results, historical context, and predictions. Includes copy-to-clipboard commands and auto-launch checklist. Ready to attach to any MR or stakeholder email.", focus: "Report is fully self-contained HTML with embedded styles. Share it as a file — no server needed. The ⬇ button is always visible in the sidebar." },
  { icon: "🏁", title: "You've Seen Every View", subtitle: "All 9 views, all 4 queries", description: "You just walked through: Problem → Impact → Verdict → Orbit Graph (NEIGHBORS) → Risk (AGGREGATION) → Path Trace → Digital Twin → Simulation → History (TRAVERSAL) → Closed-Loop Predictions → Duo Agent → Setup Wizard → Report → Export. Every Orbit query type powers a live view. Not one line of CI changed.", focus: "Press Space for auto-demo. Arrow keys navigate. D toggles dark/light theme. ⬇ exports a full HTML report. 👑 restarts this tour. Share the link with ?judge=true." },
];
