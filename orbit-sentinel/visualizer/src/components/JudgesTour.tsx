...
const TOUR_STEPS: TourStep[] = [
  { icon: "🎯", title: "The Problem", subtitle: "This is what we solve", description: "Developers spend hours per MR checking blast radius, deployment paths, and historical failures — manually. Traditional CI/CD only checks if code builds, never if it should deploy. This costs teams hours every sprint.", focus: "Three columns: Problem → Solution → Quantified Impact. Reads left to right." },
  { icon: "📈", title: "Impact by the Numbers", subtitle: "Quantified results", description: "Orbit Sentinel saves hours per MR. Fewer false alarms than CI-only alerts. Actionable remediation for every risk found. Cross-references all 4 Orbit query types. (Tip: press D or click ☀️ to toggle light theme.)", focus: "Five quantified metrics — each driven by Orbit graph data." },
  ...
  { icon: "🎯", title: "Closed-Loop Predictions", subtitle: "Post-merge verification + ROI", description: "Other tools predict risk and stop. Orbit Sentinel verifies — every prediction is tracked through a 7-day survival window. TP/TN/FP/FN confusion matrix shows real accuracy. The ROI calculator translates this into dollar impact: $139k/yr time cost saved, 121% net ROI.", focus: "Stat cards show accuracy, failures caught, and MRs tracked. Confusion matrix is color-coded. Closed-loop banner explains Predict → Ship → Verify → Learn." },
