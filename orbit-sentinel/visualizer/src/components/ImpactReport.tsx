...
function RiskGauge({ score, size = 80 }: { score: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)", filter: `drop-shadow(0 0 2px ${glow})` }}>
      ...
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--overlay-05)" strokeWidth={6} />
...
function RiskSparkline({ points }: { points: { label: string; value: number }[] }) {
  ...
  return (
    <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: "block", maxWidth: "100%" }}>
      ...
      {/* Line */}
      <path d={line} fill="none" stroke="var(--overlay-50)" strokeWidth={1.5}
  ...
  return (
    <div style={{
      ...
      <div onClick={() => setOpen(!open)} style={{
        ...
        background: open ? `${meta.color}08` : "var(--overlay-015)",
...
export default function ImpactReport({ data }: Props) {
  ...
  const handleExportMarkdown = useCallback(() => {
    const { summary, hero, decisionCenter, incidents, evidence, counterfactuals, riskData, futureTimeline } = data;
    const md = [
      `# Engineering Impact Report — MR !${summary.mrIid}`,
      ``,
      `**Project:** ${summary.project} | **Branch:** ${summary.branch}`,
      `**Risk Score:** ${summary.riskScore} (${summary.riskLevel})`,
      `**Generated:** ${new Date(summary.timestamp).toLocaleString()}`,
      ``,
      `## Verdict: ${hero.predictedOutcome}`,
      ``,
      `**Action:** ${hero.recommendedAction}`,
      ``,
      `## Risk Breakdown`,
      ...riskData.breakdown.map(b => `- **${b.category}:** ${b.value}/${b.maxValue}`),
      ``,
      `## Remediation`,
      `- Current risk: ${(decisionCenter.riskReduction.current * 100).toFixed(0)}%`,
      `- After remediation: ${(decisionCenter.riskReduction.afterRecommendation * 100).toFixed(0)}%`,
      `- Strategy: ${decisionCenter.deploymentStrategy}`,
      ``,
      `## Evidence Chain`,
      ...evidence.map(e => `- **${e.queryType}:** ${e.queryName}`),
      ``,
      incidents.length > 0 ? `## Related Incidents\n${incidents.map(i => `- !${i.mrIid} — ${i.similarity}% match (${i.outcome})`).join("\n")}` : ``,
      ``,
      `## Timeline`,
      ...futureTimeline.slice(0, 4).map(e => `- **D+${e.day}:** ${e.label} — ${e.description}`),
      ``,
      `---`,
      `*Generated by Orbit Sentinel*`,
    ].filter(Boolean).join("\n");
  ...
  const handleExportJSON = useCallback(() => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `orbit-report-${data.summary.mrIid}.json`; a.click();
        ...
        <div className="card" style={{
          ...
          <div style={{ display: "flex", gap: 4, position: "relative" }} ref={exportRef}>
            ...
            <button onClick={() => setExportOpen(!exportOpen)} aria-label="Export report" title="Export report"
              ...
              onMouseEnter={e => { if (!exportOpen) { e.currentTarget.style.color = "var(--text-primary)"; e.currentTarget.style.borderColor = "var(--border-hover)"; } }}
              onMouseLeave={e => { if (!exportOpen) { e.currentTarget.style.color = "var(--text-secondary)"; e.currentTarget.style.borderColor = "var(--border)"; } }}
            >📤</button>
            {exportOpen && (
              <div style={{
              ...
              {riskData.breakdown.map((b, i) => {
                ...
                return (
                  <div key={b.category} style={{ animation: `fadeSlideUp 0.25s ${0.1 + i * 0.03}s ease both` }}>
                    ...
                    <div style={{ height: 4, borderRadius: 2, background: "var(--overlay-04)", overflow: "hidden" }}>
