...
|----------|-------|---------|
| `VITE_API_BASE_URL` | `https://orbit-sentinel-api.onrender.com` | Points visualizer to live engine API |
...
The engine is deployed at [orbit-sentinel-api.onrender.com](https://orbit-sentinel-api.onrender.com). It will **not** return live Orbit data until `GITLAB_ACCESS_TOKEN` is set — without it, the engine runs in **fallback mode** (grep-based file analysis) and the visualizer shows a "Degraded" banner. The visualizer dashboard loads instantly with demo data and upgrades to live data when the engine responds.
